#!/usr/bin/env bash
# security-threat-model-canon-citation — the six mandatory gate-house-standard test cases (issue-10
...
DENY_CONTENT='## canon-references
#!/usr/bin/env bash
set -uo pipefail'

ALLOW_CONTENT='## canon-references
core/hooks/lib/gate-lib.sh — referenced by path, not copied.'

RA_BASE='## canon-references
#!/usr/bin/env bash
#!/usr/bin/env bash'
...
edit_replace_all_case() {
  ...
  fire "$td" "$(printf '{"tool_name":"Edit","tool_input":{"file_path":"%s","old_string":%s,"new_string":%s,"replace_all":true}}' \
    "$TARGET" "$(jstr '#!/usr/bin/env bash')" "$(jstr 'core/hooks/lib/gate-lib.sh (referenced by path)')")"
  rm -rf "$td"; report allow "$got" "Edit replace_all:true over a multiply-occurring old_string is judged on ALL occurrences"
}
edit_replace_all_case
...
multiedit_case() {
  td="$(mktd)"; 
  printf '%s\n' "$RA_BASE" > "$td/$TARGET"
  fire "$td" "$(printf '{"tool_name":"MultiEdit","tool_input":{"file_path":"%s","edits":%s}}' "$TARGET" '[{"old_string":"#!/usr/bin/env bash","new_string":"core/hooks/lib/gate-lib.sh (referenced by path)","replace_all":true},{"old_string":"## canon-references","new_string":"## canon-references","replace_all":false}]')"
  rm -rf "$td"; report allow "$got" "MultiEdit mixing replace_all true/false honours each edit own flag"
}
multiedit_case
...
kill_switch_case() { # want name value
  ...
  printf '%s' "$(write_payload "$TARGET" "$DENY_CONTENT")" \
    | env CLAUDE_PROJECT_DIR="$td" SECURITY_THREAT_MODEL_CANON_CITATION_GATE_OFF="$3" /bin/bash "$GATE" >/dev/null 2>&1
  rc=$?; case "$rc" in 0) got=allow ;; 2) got=deny ;; *) got="exit-$rc" ;; esac
  rm -rf "$td"; report "$1" "$got" "$2"
}
