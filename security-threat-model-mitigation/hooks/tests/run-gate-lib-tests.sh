#!/usr/bin/env bash
# security-threat-model-mitigation — the six mandatory gate-house-standard test cases (issue-10
...
DENY_CONTENT='## mitigation-list
we will handle it somehow'

ALLOW_CONTENT='## mitigation-list
disposition: mitigate — add input validation'

RA_BASE='## mitigation-list
disposition: mitigate — add input validation
disposition: mitigate — add rate limiting'
...
edit_replace_all_case() {
  ...
  fire "$td" "$(printf '{"tool_name":"Edit","tool_input":{"file_path":"%s","old_string":%s,"new_string":%s,"replace_all":true}}' \
    "$TARGET" "$(jstr 'mitigate')" "$(jstr 'handle')")"
...
multiedit_case() {
  td="$(mktd)"; 
  printf '%s\n' "$RA_BASE" > "$td/$TARGET"
  fire "$td" "$(printf '{"tool_name":"MultiEdit","tool_input":{"file_path":"%s","edits":%s}}' "$TARGET" '[{"old_string":"mitigate","new_string":"handle","replace_all":true},{"old_string":"input validation","new_string":"schema validation","replace_all":false}]')"
...
kill_switch_case() { # want name value
  ...
  printf '%s' "$(write_payload "$TARGET" "$DENY_CONTENT")" \
    | env CLAUDE_PROJECT_DIR="$td" SECURITY_THREAT_MODEL_MITIGATION_GATE_OFF="$3" /bin/bash "$GATE" >/dev/null 2>&1
  rc=$?; case "$rc" in 0) got=allow ;; 2) got=deny ;; *) got="exit-$rc" ;; esac
  rm -rf "$td"; report "$1" "$got" "$2"
}
