...
DENY_CONTENT='## residual-risk-note
Low residual risk remains.'

ALLOW_CONTENT='## residual-risk-note
Low residual risk remains. See docs/specs/approvers.md, Approved by JiwonJung94.'

RA_BASE='## residual-risk-note
Low residual risk on auth. Approved by JiwonJung94.
Low residual risk on logging. Approved by JiwonJung94.'
...
edit_replace_all_case() {
  ...
  fire "$td" "$(printf '{"tool_name":"Edit","tool_input":{"file_path":"%s","old_string":%s,"new_string":%s,"replace_all":true}}' \
    "$TARGET" "$(jstr 'Approved by JiwonJung94')" "$(jstr 'signed off internally')")"
...
multiedit_case() {
  td="$(mktd)"; 
  printf '%s\n' "$RA_BASE" > "$td/$TARGET"
  fire "$td" "$(printf '{"tool_name":"MultiEdit","tool_input":{"file_path":"%s","edits":%s}}' "$TARGET" '[{"old_string":"Approved by JiwonJung94","new_string":"signed off internally","replace_all":true},{"old_string":"on auth","new_string":"on authentication","replace_all":false}]')"
...
kill_switch_case() { # want name value
  ...
  printf '%s' "$(write_payload "$TARGET" "$DENY_CONTENT")" \
    | env CLAUDE_PROJECT_DIR="$td" SECURITY_THREAT_MODEL_RESIDUAL_SIGNOFF_GATE_OFF="$3" /bin/bash "$GATE" >/dev/null 2>&1
  rc=$?; case "$rc" in 0) got=allow ;; 2) got=deny ;; *) got="exit-$rc" ;; esac
  rm -rf "$td"; report "$1" "$got" "$2"
}
