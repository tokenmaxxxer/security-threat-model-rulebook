#!/usr/bin/env bash
# security-threat-model-stride — the seven mandatory gate-house-standard test
# cases (issue-10 proposal s8; 7th group added issue-13, mirroring core
# #75's own bump to seven groups), run against this plugin's own gate as a
# real subprocess.
#
...
DENY_CONTENT='## stride-table
- an untagged threat row with no category'

ALLOW_CONTENT='## asset-inventory
Web app, DB.
## trust-boundary-map
Internet -> LB -> app -> DB.
## stride-table
...
Internet -> LB -> app -> DB.
## stride-table
- Spoofing: session token forgery.
- Spoofing: session cookie forgery.'
...
edit_replace_all_case() {
  ...
  fire "$td" "$(printf '{"tool_name":"Edit","tool_input":{"file_path":"%s","old_string":%s,"new_string":%s,"replace_all":true}}' \
    "$TARGET" "$(jstr 'Spoofing')" "$(jstr 'unclassified')")"
...
multiedit_case() {
  td="$(mktd)"; 
  printf '%s\n' "$RA_BASE" > "$td/$TARGET"
  fire "$td" "$(printf '{"tool_name":"MultiEdit","tool_input":{"file_path":"%s","edits":%s}}' "$TARGET" '[{"old_string":"Spoofing","new_string":"unclassified","replace_all":true},{"old_string":"Web app, DB.","new_string":"Web app, DB, cache.","replace_all":false}]')"
...
kill_switch_case() { # want name value
  ...
  printf '%s' "$(write_payload "$TARGET" "$DENY_CONTENT")" \
    | env CLAUDE_PROJECT_DIR="$td" SECURITY_THREAT_MODEL_STRIDE_GATE_OFF="$3" /bin/bash "$GATE" >/dev/null 2>&1
  rc=$?; case "$rc" in 0) got=allow ;; 2) got=deny ;; *) got="exit-$rc" ;; esac
  rm -rf "$td"; report "$1" "$got" "$2"
}
...
# ---------------------------------------------------------------------------
# security-threat-model-stride regressions (issue-10 proposal s4 / s5)
# ---------------------------------------------------------------------------

# s4: the third pairwise ordering. Before issue-10 only
# asset-inventory -> stride-table and trust-boundary-map -> stride-table were
# enforced, so an asset inventory placed AFTER the trust-boundary map passed.
S4_BAD='## trust-boundary-map
Internet -> LB -> app -> DB.
## asset-inventory
Web app, DB.
## stride-table
- Spoofing: attacker forges a session token.'
S4_GOOD='## asset-inventory
Web app, DB.
## trust-boundary-map
Internet -> LB -> app -> DB.
## stride-table
...
# s5.1: a marker counts only as a heading or a line-start `token:` field
# marker. A mid-sentence prose mention used to satisfy find_marker's "bare
# occurrence anywhere" fallback and drag the document into the gate's scope.
S51_PROSE='# issue-10 remediation record

관련 stride-table 논의는 나중에 별도 이슈로 다룬다. No table is written
here, and no asset-inventory or trust-boundary-map heading exists.'
S51_REAL='## stride-table
- Spoofing: attacker forges a session token.'
write_case allow "s5.1 regression: mid-sentence stride-table mention is prose, not a marker" "$S51_PROSE"
write_case deny  "s5.1: a real stride-table heading with no preconditions still denies"      "$S51_REAL"

# s5.2: per-row tagging. Before issue-10 one tagged row anywhere in the
# section satisfied the whole table.
S52_BAD='## asset-inventory
Web app, DB.
## trust-boundary-map
Internet -> LB -> app -> DB.
## stride-table
...
| unclassified thing | | needs triage |
| log wipe | Repudiation | medium |'
S52_GOOD='## asset-inventory
Web app, DB.
## trust-boundary-map
Internet -> LB -> app -> DB.
## stride-table
...
Internet -> LB -> app -> DB.
## stride-table
Prose only; the rows land in a follow-up issue.'
