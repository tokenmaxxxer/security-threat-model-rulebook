#!/usr/bin/env bash
# security-threat-model — the seven mandatory gate-house-standard test cases
# (issue-10 proposal s8; 7th group added issue-13, mirroring core #75's own
# bump to seven groups), run against this plugin's own gate as a real
# subprocess.
#
...
TARGET=docs/issue-999/proposals/security-threat-model.md

mktd() { _td="$(cd "$(mktemp -d)" && pwd -P)"; git init -q "$_td"; mkdir -p "$_td/$(dirname "$TARGET")"; printf '%s' "$_td"; }
seed_allow() { mkdir -p "$1/docs/issue-999/reports/security-threat-model"; echo survey > "$1/docs/issue-999/reports/security-threat-model/survey.md"; }
...
DENY_CONTENT='phase-1 proposal body written before the survey exists.'

ALLOW_CONTENT='phase-1 proposal body written after the survey exists.'

RA_BASE='an existing proposal body. an existing proposal body.'

# ---------------------------------------------------------------- baseline
write_case deny  "baseline: a would-be-refused write denies" "$DENY_CONTENT"
write_case allow "baseline: a conforming write allows"       "$ALLOW_CONTENT" seed
...
edit_replace_all_case() {
  ...
  fire "$td" "$(printf '{"tool_name":"Edit","tool_input":{"file_path":"%s","old_string":%s,"new_string":%s,"replace_all":true}}' \
    "$TARGET" "$(jstr 'an existing proposal body.')" "$(jstr 'a rewritten proposal body.')")"
...
multiedit_case() {
  td="$(mktd)"; 
  printf '%s\n' "$RA_BASE" > "$td/$TARGET"
  fire "$td" "$(printf '{"tool_name":"MultiEdit","tool_input":{"file_path":"%s","edits":%s}}' "$TARGET" '[{"old_string":"an existing proposal body.","new_string":"first rewrite.","replace_all":true},{"old_string":"first rewrite.","new_string":"second rewrite.","replace_all":false}]')"
...
kill_switch_case() { # want name value
  ...
  printf '%s' "$(write_payload "$TARGET" "$DENY_CONTENT")" \
    | env CLAUDE_PROJECT_DIR="$td" SECURITY_THREAT_MODEL_SEQUENCE_GATE_OFF="$3" /bin/bash "$GATE" >/dev/null 2>&1
  rc=$?; case "$rc" in 0) got=allow ;; 2) got=deny ;; *) got="exit-$rc" ;; esac
  rm -rf "$td"; report "$1" "$got" "$2"
}
...
# for the guard added at sequence-gate.sh:2 (core #75's guard shape,
# issue-13): before the guard, the unguarded source failed but
# gate_trap_fail_closed was never reached to force the deny, so the gate
# fell through and allowed.
missing_core_case() {
  td="$(mktd)"
  printf '%s' "$(write_payload "$TARGET" "$DENY_CONTENT")" \
